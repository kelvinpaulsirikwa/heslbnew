            ...
            <div class="col-lg-8 col-12">
                ...
                <div class="bg-white shadow-sm border rounded-3 p-4 mb-4">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                        <div>
                            <h2 class="mb-1 text-dark fw-bold">Edit FAQ</h2>
                            <p class="text-muted mb-0 small">Update existing frequently asked question</p>
                        </div>
                    ...
                    <form action="{{ route('faq.update', $faq->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="steps-wrapper" class="border rounded-3 p-3 bg-light bg-opacity-50">
                                @php
                                    $steps = old('steps', json_decode($faq->answer, true) ?? []);
                                @endphp
                                @if(count($steps))
                                    @foreach($steps as $index => $step)
                                        <div class="d-flex align-items-center mb-2">
                                            <input type="text" name="steps[]" class="form-control me-2" 
                                ...
                                @else
                                    <div class="d-flex align-items-center mb-2">
                                        <input type="text" name="steps[]" class="form-control me-2" 
                                               placeholder="Step 1: Enter step description..." required>
                                    </div>
                                @endif
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="type" name="type" class="form-select form-select-lg" required>
                                ...
                                <option value="loan_application" {{ old('type', $faq->type) == 'loan_application' ? 'selected' : '' }}>Loan Application</option>
                                <option value="loan_repayment" {{ old('type', $faq->type) == 'loan_repayment' ? 'selected' : '' }}>Loan Repayment</option>
                            </select>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('add-step').addEventListener('click', function() {
            ...
            removeBtn.className = 'btn btn-outline-danger btn-sm remove-step';
        ...
        // Initialize remove events for existing steps
        attachRemoveEvents();
    });
</script>
